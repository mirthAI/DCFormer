...
def get_args_parser():
    ...
    parser.add_argument('--epochs', default=17, type=int)
    ...
    parser.add_argument('--file', default='ctvit/',
                        help='path where to save, empty for no saving')
    parser.add_argument('--file_save', default='ctvit/',
                        help='path where to save, empty for no saving')
...
class ImageLatentsClassifier(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        image_latents, _ = self.trained_model(*args, **kwargs)
...
def main(args):
    ...
    dataset = DATASET(      args,
                            ...
                            mae=args.mae,
                            # train_latent=args.data_path + args.file + 'train/',
                            # test_latent=args.data_path + args.file + 'test/',
    ...
    weights = torch.tensor([9.211362733, 2.384068466, 8.295479204, 32.8629776, 2.992233613,
                            ...
                            6.335045662, 10.81701149, 13.40695067]).to(device)
    ...
    clip = misc.load_pretrained(args, clip)

    # checkpoint = torch.load(args.resume, map_location='cpu')
    # clip.load_state_dict(checkpoint['model'])
...
if __name__ == '__main__':
    ...
    args.output_dir += 'finetune/' + args.file_save + '/'
    args.model_dir +=  args.file + '/'
    if args.output_dir:
