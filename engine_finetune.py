...
def train_one_epoch(model: torch.nn.Module, tokenizer,
    ...
    for data_iter_step, (nii_file, img, onehotlabels)in enumerate(metric_logger.log_every(data_loader, print_freq, header)):

        # we use a per iteration (instead of per epoch) lr scheduler
        if args.scheduler:
            if data_iter_step % accum_iter == 0:
                lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader) + epoch, args)
        text_tokens = tokenizer([" "], return_tensors="pt", padding="max_length", truncation=True, max_length=512)

        img, text_tokens, onehotlabels = img.to(device), text_tokens.to(device), onehotlabels.float().to(device)
        # img, text_tokens = img.to(device, non_blocking=True), text_tokens.to(device, non_blocking=True)
        with torch.amp.autocast('cuda'):
            logits = model(img, text_tokens, device)

        loss = criterion(logits, onehotlabels)
...
def evaluate(epoch, data_loader, model, tokenizer, device, plot_dir):
    ...
    for data_iter_step, (nii_file, img, onehotlabels) in enumerate(data_loader):
        ...
        img = img.to(device)
        onehotlabels = onehotlabels.float().to(device)

        with torch.amp.autocast('cuda'):
            text_tokens = tokenizer("", return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(device)

            output = model(img, text_tokens,  device=device)
            realall.append(onehotlabels.detach().cpu().numpy()[0])                                

            save_out = sigmoid(torch.tensor(output)).detach().cpu().numpy()
