...
def params(m, x):
    flops, params = flopth(m, inputs=(x, ))
    print(flops, params)
...
# def DecomposedStem(inp, oup, image_size, downsample=False):
#     stride = 1 if downsample == False else 2
#     return nn.Sequential(
#             DecompConv3D(inp, oup, 3, stride, 1, nn.GELU()),
#             DecompConv3D(oup, oup, 3, 1, 1, nn.GELU())
...
class DecompConv3D(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride=1, groups=1, norm=True, act=None) -> None:
        super().__init__()
        self.act = act

        self.norm = nn.ModuleList([nn.BatchNorm3d(out_dim) if norm is not None else nn.Identity() for _ in range(3)])


        self.c1 = nn.Sequential(
                                nn.Conv3d(in_dim, out_dim, kernel_size=(kernel_size, 1, 1), padding=(kernel_size//2, 0, 0), stride=stride, groups=groups),
                                self.norm[0])
        self.c2 = nn.Sequential(
                                nn.Conv3d(in_dim, out_dim, kernel_size=(1, kernel_size, 1), padding=(0, kernel_size//2, 0), stride=stride,groups=groups),
                                self.norm[1])
        self.c3 = nn.Sequential(
                                nn.Conv3d(in_dim, out_dim, kernel_size=(1, 1, kernel_size), padding=(0, 0, kernel_size//2), stride=stride, groups=groups),
                                self.norm[2])
...
class MLP(nn.Module):
    def __init__(self, oup, mlp_dim, dp=0.):
        super().__init__()
...
class DecomposedAttention(nn.Module):
    ...
    def forward(self, x, size):
        ...
        assert n == h * w * t
        B_, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, N, 3, self.head_num, C // self.head_num).permute(2, 0, 3, 1, 4)
...
class ChannelBlock(nn.Module):
    def __init__(self, dim, heads=8):
        ...
        self.cpe = nn.ModuleList([ConvPosEnc(dim=dim, k=3, decompose=True),
...
class TransformerBlock(nn.Module):
    def __init__(self, inp, oup, image_size, kernel_size, heads=8, dim_head=32, downsample=False, dropout=0.):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, input_size, in_channels, num_blocks, channels, kernel_sizes=[7, 7, 7, 7], block_types=['C', 'C', 'C', 'C']):
        super().__init__()
        self.dims = channels
        ih, iw, it = input_size
        ...
        self.s0 = self._make_layer(
            DecomposedStem, in_channels, channels[0], num_blocks[0],kernel_sizes[0], (ih // i, iw // i, it // i))
        self.s1 = self._make_layer(
            block[block_types[0]], channels[0], channels[1], num_blocks[1], kernel_sizes[0], (ih // (i * 2 ** 1), iw // (i * 2 ** 1), it // (i * 2 ** 1)))
        self.s2 = self._make_layer(
            block[block_types[1]], channels[1], channels[2], num_blocks[2], kernel_sizes[1], (ih // (i * 2 ** 2), iw // (i * 2 ** 2), it // (i * 2 ** 2)))
        self.s3 = self._make_layer(
            block[block_types[2]], channels[2], channels[3], num_blocks[3], kernel_sizes[2], (ih // (i * 2 ** 3), iw // (i * 2 ** 3), it // (i * 2 ** 3)))
        self.s4 = self._make_layer(
            block[block_types[3]], channels[3], channels[4], num_blocks[4], kernel_sizes[3], (ih // (i * 2 ** 4), iw // (i * 2 ** 4), it // (i * 2 ** 4)))
...
class DecompModel(nn.Module):
    def __init__(self, 
        ...
        self.encoder = Encoder(input_size, in_channels, num_blocks, channels, kernel_sizes, block_types)    
...
if __name__ == '__main__':
    ...
    x = torch.rand(1, 1, 256, 512, 512)
    # x = torch.rand(1, 1, 128, 256, 256)
    # out = model(x)
    # params(model.encoder, x)
    flops = FlopCountAnalysis(model.encoder, x)
    # flops_str = flop_count_str(flops)
    table = flop_count_table(flops)
    # print(flops_str)
    print(table)
    print('PARAMS: ', readable_params(sum(p.numel() for p in model.parameters() if p.requires_grad)))
    print('FLOPS: ', readable_flops(flops.total()))