...
def evaluate_internal(y_pred, y_true, cxr_labels, plot_dir,
    ...
    for i in range(num_classes):
        ...
        acc = accuracy_score(np.rint(y_true_i).astype(int), np.rint(y_pred_i).astype(int))
        f1 = f1_score(np.rint(y_true_i).astype(int), np.rint(y_pred_i).astype(int))
        pr = precision_score(np.rint(y_true_i).astype(int), np.rint(y_pred_i).astype(int))
        rec = recall_score(np.rint(y_true_i).astype(int), np.rint(y_pred_i).astype(int))
        ...
        # pr, rec, thresholds = plot_pr(y_pred_i, y_true_i, pr_name, plot_dir, plot=False)

        df = pd.DataFrame([roc_auc], columns=[cxr_label+'_auc'])
        dataframes.append(df)
        dfa = pd.DataFrame([acc], columns=[cxr_label+'_acc'])
        dataframesa.append(dfa)
        dff = pd.DataFrame([f1], columns=[cxr_label+'_f1'])
        dataframesf.append(dff)
        dfpr = pd.DataFrame([pr], columns=[cxr_label+'_pr'])
        dataframespr.append(dfpr)
        dfrec = pd.DataFrame([rec], columns=[cxr_label+'_rec'])
        dataframesrec.append(dfrec)
        dfspe = pd.DataFrame([spec], columns=[cxr_label+'_spe'])
        dataframespe.append(dfspe)
    ...
    dfsa = pd.concat(dataframesa, axis=1)
    dfsf = pd.concat(dataframesf, axis=1)
    dfspr = pd.concat(dataframespr, axis=1)
    dfsrec = pd.concat(dataframesrec, axis=1)
    dfspe = pd.concat(dataframespe, axis=1)

    return [dfs, dfsa, dfsf, dfspr, dfsrec, dfspe]
